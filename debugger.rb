...
statu_values = [
  Tk::Tile::Label.new(status_area) {textvariable $cycle; width 8},
  Tk::Tile::Label.new(status_area) {textvariable $a; width 8},
  Tk::Tile::Label.new(status_area) {textvariable $x; width 8},
  Tk::Tile::Label.new(status_area) {textvariable $y; width 8},
  Tk::Tile::Label.new(status_area) {textvariable $p; width 10},
  Tk::Tile::Label.new(status_area) {textvariable $pc; width 8},
  Tk::Tile::Label.new(status_area) {textvariable $sp; width 8},
  Tk::Tile::Label.new(status_area) {textvariable $instruction; width 10},
]
...
##buttons
bottons = [
  Tk::Tile::Button.new($root) {text 'Assemble'; command {assemble} }.grid(:sticky => 'w'),
  Tk::Tile::Button.new($root) {text 'Run by step'; command {run_by_step} }.grid(:sticky => 'w'),
  Tk::Tile::Button.new($root) {text 'Run'; command {run} }.grid(:sticky => 'w')
]
...
page_selector.bind('<ComboboxSelected>') { change_page }
